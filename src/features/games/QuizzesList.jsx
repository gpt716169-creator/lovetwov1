...
const QuizzesList = () => {
    ...
    useEffect(() => {
        ...
        // Polling every 5s (less aggressive)
        const interval = setInterval(() => {
            if (profile?.id) fetchAttempts();
        }, 5000);

        return () => clearInterval(interval);
    }, [profile?.id]); // Use profile.id as dependency specific
    ...
    const fetchAttempts = async () => {
        ...
        else if (data) setActiveAttempts(data);
    };
    ...
    const runAiAnalysis = async (attempt) => {
        ...
        try {
            ...
            const quizPoints = attempt.quiz.questions.map((q, i) => {
                ...
                const safeOptions = q.options || ["Вариант 1", "Вариант 2"]; // Safeguard against missing options
                const initAns = (initAnsIdx !== undefined && initAnsIdx !== null) ? safeOptions[initAnsIdx] : "НЕ ОТВЕТИЛ";
                const partGuess = (partGuessIdx !== undefined && partGuessIdx !== null) ? safeOptions[partGuessIdx] : "НЕ ОТВЕТИЛ";
...
- ${partnerName} (пытался УГАДАТЬ выбор ${initiatorName}): "${partGuess}"`;
            ...
            ИМЕНА УЧАСТНИКОВ:
            - Инициатор (отвечал о себе): ${attempt.initiator?.first_name || 'Инициатор'}
            - Партнер (угадывал ответы Инициатора): ${attempt.partner?.first_name || 'Партнер'}
            ...
            3. Если ответы не совпадают, это значит, что ${attempt.partner?.first_name || 'Партнер'} ошибся в догадке.
            ...
            2. Обращайся к участникам ПО ИМЕНАМ. Не пиши "Партнер" или "Инициатор", используй их реальные имена.
            3. Дай конкретный совет паре, основанный на их ответах.

            ФОРМАТ ОТВЕТА (JSON):
            {
                "summary": "Краткое резюме (1-2 предложения) с использованием имен.",
                "detailed_analysis": {
                    "matches": { "1": "${attempt.partner?.first_name} отлично знает, что ${attempt.initiator?.first_name} любит...", "2": "..." },
                    "mismatches": { "1": "${attempt.partner?.first_name} думал(а), что..., но ${attempt.initiator?.first_name} выбрал(а)...", "2": "..." },
            ...
            const runAiAnalysis = async (attempt) => {
                ...
                try {
                    ...
                    const quizPoints = attempt.quiz.questions.map((q, i) => {
                        ...
                        return `Вопрос ${i + 1}: "${q.question}"
- ${attempt.initiator?.first_name || 'Инициатор'} (отвечал про себя): "${initAns}"
- ${attempt.partner?.first_name || 'Партнер'} (пытался УГАДАТЬ выбор Инициатора): "${partGuess}"`;
                    }).join('\n\n');
            ...
            ВАЖНО ПОНИМАТЬ КОНТЕКСТ:
            1. Инициатор отвечал на вопросы о СЕБЕ и СВОИХ предпочтениях.
            2. Партнер пытался УГАДАТЬ, что выбрал Инициатор.
            3. Если ответы не совпадают, это значит, что Партнер ошибся в своих предположениях о предпочтениях Инициатора. Это НЕ значит, что Партнер выбрал бы другое для себя, он просто не угадал.

            КОНТЕКСТ И ОТВЕТЫ (ВНИМАНИЕ: если написано "НЕ ОТВЕТИЛ", значит данных нет, учитывай это):
            ${quizPoints}

            ЗАДАЧА:
            1. Проанализируй, насколько хорошо Партнер знает Инициатора (насколько точны были его догадки).
            2. Выдели темы, где возникло недопонимание (где Партнер не смог угадать предпочтения Инициатора).
            3. Дай добрый, конкретный совет паре, как улучшить взаимопонимание в тех сферах, где догадки были неверными.

            ФОРМАТ ОТВЕТА (JSON):
            {
                "summary": "Общее резюме: насколько хорошо партнеры знают друг друга (1-2 предложения)",
                "detailed_analysis": {
                    "matches": { "1": "Здесь Партнер верно угадал...", "2": "Хорошее знание привычек..." },
                    "mismatches": { "1": "Партнер думал, что..., а на самом деле...", "2": "Стоит обсудить эту тему..." },
                    "advice": "Твой совет для улучшения понимания друг друга..."
                }
            }
            `;
